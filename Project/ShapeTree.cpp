...
std::vector<Node *> ShapeTree::applyRules(Node *current, std::vector<std::function<std::vector<Geometry>(Geometry)>> rules) {
    std::vector<Geometry> successors;
    for (int i = 0; i < rules.size(); i++) {
        std::vector<Geometry> result = rules[i](current->getShape());
...
void ShapeTree::buildTree(std::vector<std::function<std::vector<Geometry>(Geometry)>> rules, Geometry &axiom) {
...
std::vector<Geometry> ShapeTree::getLeafNodes(){
